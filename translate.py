...
def greedy_decode(model, src, max_len, start_symbol, config, data_loader):
    ...
    memory = model.encoder(src)  # 对输入的Token序列进行编码得到memory，就是encoder的输出
    ...
    for i in range(max_len - 1):  # 逐时刻进行解码
        ...
        _, next_word = torch.max(prob, dim=1)  # 选择概率最大者 ，当前最后这个时刻的预测结果
        next_word = next_word.item()
        ys = torch.cat([ys, torch.ones(1, 1).type_as(src.data).fill_(next_word)], dim=0)
        # 将当前时刻解码的预测输出结果，同之前所有的结果堆叠作为输入再去预测下一个词。
        if next_word == data_loader.EOS_IDX:  # 如果当前时刻的预测输出为结束标志（<eos>），则跳出循环结束预测。
            break
    return ys
...
def translate(model, src, data_loader, config):
    # src指的是一句话（一句德文）
    # predict
    src_vocab = data_loader.de_vocab    # 先拿到训练时候建的词表
    tgt_vocab = data_loader.en_vocab
    src_tokenizer = data_loader.tokenizer['de']
    model.eval()
    tokens = [src_vocab[tok] if tok in src_vocab else src_vocab['<unk>'] for tok in src_tokenizer(src)]  # 构造一个样本（把原始token id化）
    num_tokens = len(tokens)
    src = (torch.LongTensor(tokens).reshape(num_tokens, 1))  # 将src_len 作为第一个维度 => [src_len, 1]
    with torch.no_grad():
    ...
    # 最后将生成的结果拼接成字符串的时候，要将开头的<bos>和<eos>去掉
