...
def accuracy(logits, y_true, PAD_IDX):
    ...
    # 首先将 [tgt_len, batch_size, tgt_vocab_size] 转成 [batch_size, tgt_len, tgt_vocab_size]
    # 其次取第二个维度的最大值的索引得到形状为 [batch_size, tgt_len], 然后再转成 [batch_size * tgt_len]
    y_true = y_true.transpose(0, 1).reshape(-1)
    # [tgt_leb, batch_size] => [batch_size, tgt_len] => [batch_size * tgt_len]
...
def train_model(config):
    ...
    for epoch in range(config.epochs):
        ...
        for idx, (src, tgt) in enumerate(train_iter):
            src = src.to(config.device)  # [src_len, batch_size] 德语输入
            tgt = tgt.to(config.device)
            tgt_input = tgt[:-1, :]  # 解码部分的输入（要把最后的eos标记去掉）, [tgt_len, batch_size]

            ##=====================================
            ## 这几个mask矩阵一定要弄懂干什么用，并且shape是多少
            src_mask, tgt_mask, src_padding_mask, tgt_padding_mask \
                = data_loader.create_mask(src, tgt_input, config.device)
            # [src_len, src_len], [tgt_len, tgt_len], [batch_size, src_len], [batch_size, tgt_len]
            # =====================================
